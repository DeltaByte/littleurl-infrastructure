# binary aliases
TF = terraform -chdir=./infrastructure/

# variables
TF_PLAN = ./infrastructure.tfplan

##@ Dependencies
.PHONY: install install-tools

install: install-tools ## Install all dependencies

install-tools: ## Install tooling.
	asdf install

##@ Lint
.PHONY: lint lint-fix lint-tf lint-fix-tf

lint: lint-tf ## Run all linters

lint-fix: lint-fix-tf ## Attempt to automagically fiax all linting issues

lint-tf: ## Lint terraform code
	@echo "Linting Terraform"
	$(TF) init -backend=false
	$(TF) validate
	$(TF) fmt -check -recursive

lint-fix-tf: ## Auto fix terraform linting errors
	@echo "Fixing Terraform linting"
	$(TF) init -backend=false
	$(TF) fmt -recursive

##@ Deployment
.PHONY: tf-init tf-plan tf-apply

tf-init: ## Initialise terraform
	@echo "Initialising terraform"
	$(TF) init

tf-plan: ## Plan terraform changeset
	@echo "Creating terraform plan"
	$(TF) plan -input=false -out=${TF_PLAN}

tf-apply: ## Apply terraform changeset
	@echo "Applying terraform plan"
	$(TF) apply -input=false -auto-approve ${TF_PLAN}

##@ Helpers
.PHONY: help

help: ## Display this help.
	@awk 'BEGIN {FS = ":.*##"; printf "\nUsage:\n  make \033[36m<target>\033[0m\n"} /^[a-zA-Z_-]+:.*?##/ { printf "  \033[36m%-15s\033[0m %s\n", $$1, $$2 } /^##@/ { printf "\n\033[1m%s\033[0m\n", substr($$0, 5) } ' $(MAKEFILE_LIST)